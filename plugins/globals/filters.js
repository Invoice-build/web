...
export const integer = val => {
  ...
  if (!isNumberCheck(val)) return parseFloat(0.0).toLocaleString(locale, { maximumFractionDigits: 0 })
...
export const datetime = val => {
  ...
  const parsed = new Date(val)
  return parsed.toLocaleString(locale, { month: 'short', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false, timeZoneName: 'short' })
}
